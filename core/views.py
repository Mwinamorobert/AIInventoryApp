...
def insights_view(request):
    ...
    if not anomalies_df.empty:
        ...
        empty_anomalies_df = pd.DataFrame({'name': [], 'quantity': []})
        anomalies_fig = px.bar(empty_anomalies_df, x='name', y='quantity', title='No Anomalies Detected')

    # ----- Stock Trends -----
    stock_data_qs = items.values('name').annotate(total=Sum('quantity')).order_by('-total')
    stock_df = pd.DataFrame(list(stock_data_qs))
    if not stock_df.empty and 'name' in stock_df.columns and 'total' in stock_df.columns:
        stock_df['total'] = pd.to_numeric(stock_df['total'], errors='coerce')
        stock_df = stock_df.dropna(subset=['total'])
        stock_fig = px.bar(stock_df, x='name', y='total', title='Stock Trends')
    else:
        empty_stock_df = pd.DataFrame({'name': [], 'total': []})
        stock_fig = px.bar(empty_stock_df, x='name', y='total', title='Stock Trends (No Data)')

    # ----- Text Insights -----
    low_stock = [
        {'name': item.name, 'quantity': item.quantity, 'location': item.location.name if item.location else 'N/A'}
        for item in items if item.quantity < 10
    ]
    restock_suggestions = [
        {'name': item.name, 'suggested_quantity': max(100 - item.quantity, 0)}
        for item in items if item.quantity < 50
    ]
